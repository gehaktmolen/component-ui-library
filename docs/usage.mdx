...
Learn the basics of working with Azrn UI components.

## Responsive meta tag

The library is a _mobile-first_ component library—we write code for mobile devices first, and then scale up the components as necessary using CSS media queries.
...
The components are self-supporting and fully functional in isolation.
...
For example, the Badge contains two slots:
...
For example, the Badge component renders a `<span>` by default.
...
If you are customizing a component like the [Button](?path=/docs/inputs-button--docs) that only has a root slot, you may prefer to use the more succinct `component` prop instead of `slots`.
...
The library includes two kinds of building blocks: **components** and **hooks**.

Hooks encapsulate _logic_; components provide _structure_.

Many components are implemented with the help of hooks.
