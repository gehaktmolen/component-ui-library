...
Azrn UI components are designed to suit the widest possible range of use cases, but you may occasionally need to change how a component's structure is rendered in the DOM.
...
For example, the Button's root slot is a `<button>` element. This component only has a root slot; more complex components may have additional interior slots.
