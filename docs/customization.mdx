...
The `toggle:thumb` and `toggle:class` are added unconditionally—they will always be present on the Toggle component.
...
```ts
{
    ...
    getInputProps: (otherProps?: object) => ToggleInputProps;
}
```
