...
import { useGridRegisterPipeProcessor, GridPipeProcessor, GridStateInitializer } from '../../../../DataTable/internals';
...
import { GridDetailPanelApi, GridDetailPanelPrivateApi, GridDetailPanelState } from './gridDetailPanelInterface';
...
function cacheContentAndHeight(
    ...
    const contentCache = rowIds.reduce<Record<GridRowId, ReturnType<typeof getDetailPanelContent>>>((acc, id) => {
...
export const useGridDetailPanel = (
    ...
    const storeDetailPanelHeight = React.useCallback<GridDetailPanelPrivateApi['storeDetailPanelHeight']>(
        (id, height) => {
    ...
    const detailPanelHasAutoHeight = React.useCallback<GridDetailPanelPrivateApi['detailPanelHasAutoHeight']>(
        (id) => {
            const heightCache = gridDetailPanelRawHeightCacheSelector(apiRef.current.state);
    ...
    const previousGetDetailPanelContentProp = React.useRef<DataGridProProcessedProps['getDetailPanelContent']>();
    const previousGetDetailPanelHeightProp = React.useRef<DataGridProProcessedProps['getDetailPanelHeight']>();
