...
import { GridColumnPinningApi, GridPinnedPosition, GridPinnedColumns } from './gridColumnPinningInterface';
...
export const useGridColumnPinning = (
    ...
    props: Pick<
        ...
        'disableColumnPinning' | 'initialState' | 'pinnedColumns' | 'onPinnedColumnsChange' | 'slotProps' | 'slots'
    >
): void => {
    ...
    const updateHoveredClassOnSiblingRows = React.useCallback(
        (event: React.MouseEvent<HTMLElement>) => {
            ...
            const rowElements = rowContainer.querySelectorAll(`.row[data-rowindex="${index}"]`);
            rowElements.forEach((row) => {
                // Ignore rows from other grid inside the hovered row
                if (row.closest(`.virtualScroller`) === apiRef.current.virtualScrollerRef!.current!) {
    ...
    const calculateScrollLeft = React.useCallback<GridPipeProcessor<'scrollToIndexes'>>(
        (initialValue, params) => {
            ...
            const [leftPinnedColumns, rightPinnedColumns] = filterColumns(pinnedColumns, visibleColumnFields, true);
    ...
    const checkIfEnabled = React.useCallback(
        (methodName: keyof GridColumnPinningApi) => {
            if (props.disableColumnPinning) {
                throw new Error(
                    `Azrn: You cannot call \`apiRef.current.${methodName}\` when \`disableColumnPinning\` is true.`
                );
            }
        },
        [props.disableColumnPinning]
