...
import { GridStateInitializer } from '../../../../DataTable/internals';
...
const getCursorMoveDirectionX = (currentCoordinates: CursorCoordinates, nextCoordinates: CursorCoordinates) => {
    return currentCoordinates.x <= nextCoordinates.x ? CURSOR_MOVE_DIRECTION_RIGHT : CURSOR_MOVE_DIRECTION_LEFT;
};

const hasCursorPositionChanged = (currentCoordinates: CursorCoordinates, nextCoordinates: CursorCoordinates): boolean =>
...
export const useGridColumnReorder = (
    ...
    props: Pick<
        ...
        'disableColumnReorder' | 'keepColumnPositionIfDraggedOutside' | 'classes' | 'onColumnOrderChange'
    >
): void => {
    ...
    const handleDragEnter = React.useCallback<GridEventListener<'cellDragEnter' | 'columnHeaderDragEnter'>>(
        (params, event) => {
    ...
    const handleDragOver = React.useCallback<GridEventListener<'cellDragOver' | 'columnHeaderDragOver'>>(
        (params, event) => {
            ...
            if (params.field !== dragColField && hasCursorPositionChanged(cursorPosition.current, coordinates)) {
                ...
                const hasMovedLeft =
                    cursorMoveDirectionX === CURSOR_MOVE_DIRECTION_LEFT && dragColIndex < targetColIndex;
                const hasMovedRight =
                    cursorMoveDirectionX === CURSOR_MOVE_DIRECTION_RIGHT && targetColIndex < dragColIndex;
                ...
                if (hasMovedLeft || hasMovedRight) {
                    ...
                    if (!targetCol.disableReorder) {
                        ...
                        canBeReordered =
                            targetColVisibleIndex > 0 && !visibleColumns[targetColVisibleIndex - 1].disableReorder;
                    } else {
                        canBeReordered =
