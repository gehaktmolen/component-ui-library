...
import { ownerDocument, useEventCallback } from '../../../../../utils';
import {
    GridEventListener,
    CursorCoordinates,
...
function computeOffsetToSeparator(clickX: number, columnBounds: DOMRect, resizeDirection: ResizeDirection) {
...
function getResizeDirection(element: HTMLElement, direction: Direction) {
    const side = element.classList.contains('columnSeparator--sideRight') ? 'Right' : 'Left';
    if (direction === 'rtl') {
...
export const useGridColumnResize = (
    ...
    const handleColumnResizeMouseDown: GridEventListener<'columnSeparatorMouseDown'> = useEventCallback(
        ({ colDef }, event) => {
            ...
            if (!event.currentTarget.classList.contains('columnSeparator--resizable')) {
                return;
            }
            ...
            resizeDirection.current = getResizeDirection(event.currentTarget, 'rtl');
    ...
    const handleTouchStart = useEventCallback((event: any) => {
        const cellSeparator = findParentElementFromClassName(event.target, 'columnSeparator--resizable');
        ...
        colElementRef.current = findParentElementFromClassName(event.target, 'columnHeader') as HTMLDivElement;
        ...
        resizeDirection.current = getResizeDirection(event.target, 'rtl');
