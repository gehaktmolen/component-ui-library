...
const defaultItemComparer = <ItemValue>(optionA: ItemValue, optionB: ItemValue) => optionA === optionB;

const defaultIsItemDisabled = () => false;

const defaultItemStringifier = <ItemValue>(item: ItemValue) => (typeof item === 'string' ? item : String(item));
...
export function useList<
    ...
    CustomActionContext = NonNullable<unknown>
>(
    ...
    const reducer = (externalReducer ?? defaultReducer) as React.Reducer<State, ListAction<ItemValue> | CustomAction>;
    ...
    const getItemState = React.useCallback(
        (item: ItemValue): ListItemState => {
            ...
            const highlighted =
                latestHighlightedValue.current != null && itemComparer(item, latestHighlightedValue.current);
