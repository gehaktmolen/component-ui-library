...
const useUtilityClasses = () => {
    const slots = {
        ...
        actions: ['isolate inline-flex -space-x-px rounded-md shadow-sm']
    };
...
const TablePagination = React.forwardRef(function TablePagination<RootComponentType extends React.ElementType>(
    ...
    const selectLabelProps: WithOptionalOwnerState<TablePaginationSelectLabelSlotProps> = useSlotProps({
    ...
    const displayedRowsProps: WithOptionalOwnerState<TablePaginationDisplayedRowsSlotProps> = useSlotProps({
    ...
    return (
        <Root {...rootProps}>
            <Toolbar {...toolbarProps}>
                <Spacer {...spacerProps} />
                {rowsPerPageOptions.length > 1 && <SelectLabel {...selectLabelProps}>{labelRowsPerPage}</SelectLabel>}

                {rowsPerPageOptions.length > 1 && (
                    <Select {...selectProps}>
                        {rowsPerPageOptions.map((rowsPerPageOption: number | { label: string; value: number }) => (
                            <MenuItem
                                {...menuItemProps}
                                key={
...
TablePagination.propTypes = {
    ...
    page: chainPropTypes(integerPropType.isRequired, (props) => {
        ...
        if (page < 0 || page > newLastPage) {
            return new Error(
                'Azrn: The page prop of a TablePagination is out of range ' +
