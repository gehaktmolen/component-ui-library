...
export type GridPipeProcessor<P extends GridPipeProcessorGroup> = (
    ...
    context: GridPipeProcessingLookup[P] extends { context: any } ? GridPipeProcessingLookup[P]['context'] : undefined
) => GridPipeProcessingLookup[P]['value'];

type GridPipeProcessorsApplierArgs<P extends GridPipeProcessorGroup, T extends { value: any }> = T extends {
    context: any;
}
    ? [P, T['value'], T['context']]
    : [P, T['value']];
