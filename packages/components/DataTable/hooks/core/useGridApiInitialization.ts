...
export function useGridApiInitialization<PrivateApi extends GridPrivateApiCommon, Api extends GridApiCommon>(
    ...
    const publishEvent = React.useCallback<GridCoreApi['publishEvent']>(
        (...args: any[]) => {
            ...
            event.defaultAzrnPrevented = false;
            ...
            const details =
                props.signature === GridSignature.DataGridPro ? { api: privateApiRef.current.getPublicApi() } : {};
