...
export const useGridStateInitialization = <PrivateApi extends GridPrivateApiCommon>(
    ...
    const controlStateMapRef = React.useRef<Record<string, GridControlStateItem<PrivateApi['state'], any>>>({});
    const [, rawForceUpdate] = React.useState<PrivateApi['state']>();

    const registerControlState = React.useCallback<GridStatePrivateApi<PrivateApi['state']>['registerControlState']>(
        (controlStateItem) => {
        ...
        (state, reason) => {
            ...
            if (updatedControlStateIds.length === 1) {
                ...
                if (controlState.propOnChange && hasPropChanged) {
                    const details =
                        props.signature === GridSignature.DataGridPro ? { api: apiRef.current, reason } : { reason };
    ...
    const updateControlState = React.useCallback<GridStatePrivateApi<PrivateApi['state']>['updateControlState']>(
        (key, state, reason) => {
