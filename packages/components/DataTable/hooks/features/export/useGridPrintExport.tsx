...
import { GridExportDisplayOptions, GridPrintExportMenuItem } from '../../../components/toolbar/GridToolbarExport';
...
type PrintWindowOnLoad = (
    ...
    options?: Pick<GridPrintExportOptions, 'copyStyles' | 'bodyClassName' | 'pageStyle' | 'hideToolbar' | 'hideFooter'>
) => void;
...
export const useGridPrintExport = (
    ...
    const handlePrintWindowLoad: PrintWindowOnLoad = React.useCallback(
        (printWindow, options): void => {
            ...
            const gridMain: HTMLElement | null = gridClone.querySelector(`.main`);
            gridMain!.style.overflow = 'visible';

            // See https://support.google.com/chrome/thread/191619088?hl=en&msgid=193009642
            gridClone!.style.contain = 'size';

            const columnHeaders = gridClone.querySelector(`.columnHeaders`);
            const columnHeadersInner = columnHeaders!.querySelector<HTMLElement>(`.columnHeadersInner`)!;
            columnHeadersInner.style.width = '100%';

            let gridToolbarElementHeight =
                gridRootElement!.querySelector<HTMLElement>(`.toolbarContainer`)?.offsetHeight || 0;
            let gridFooterElementHeight =
                gridRootElement!.querySelector<HTMLElement>(`.footerContainer`)?.offsetHeight || 0;

            if (normalizeOptions.hideToolbar) {
                gridClone.querySelector(`.toolbarContainer`)?.remove();
                gridToolbarElementHeight = 0;
            }

            if (normalizeOptions.hideFooter) {
                gridClone.querySelector(`footerContainer`)?.remove();
            ...
            if (normalizeOptions.copyStyles) {
                ...
                const root =
                    rootCandidate.constructor.name === 'ShadowRoot' ? (rootCandidate as ShadowRoot) : doc.current;
    ...
    const exportDataAsPrint = React.useCallback<GridPrintExportApi['exportDataAsPrint']>(
        async (options) => {
            ...
            if (!apiRef.current.rootElementRef!.current) {
                throw new Error('Azrn: No grid root element available.');
            }
    ...
    const addExportMenuButtons = React.useCallback<GridPipeProcessor<'exportMenu'>>(
        (initialValue, options: { printOptions: GridPrintExportOptions & GridExportDisplayOptions }) => {
