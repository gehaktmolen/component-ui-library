...
import { GridRowSelectionApi, GridRowMultiSelectionApi } from '../../../models/api/gridRowSelectionApi';
...
export const useGridRowSelection = (
    ...
    const setRowSelectionModel = React.useCallback<GridRowSelectionApi['setRowSelectionModel']>(
        (model) => {
            if (
                ...
                throw new Error(
                    [
                        'Azrn: `rowSelectionModel` can only contain 1 item in DataGrid.',
    ...
    const handleRowClick = React.useCallback<GridEventListener<'rowClick'>>(
        (params, event) => {
            ...
            const field = (event.target as HTMLDivElement).closest(`.cell`)?.getAttribute('data-field');
    ...
    const handleRowSelectionCheckBoxChange = React.useCallback<GridEventListener<'rowSelectionCheckBoxChange'>>(
        (params, event) => {
    ...
    const handleHeaderSelectionCheckBoxChange = React.useCallback<GridEventListener<'headerSelectionCheckBoxChange'>>(
        (params) => {
