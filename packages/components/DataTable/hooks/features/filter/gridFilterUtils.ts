 ...
 */
export const cleanFilterItem = (item: GridFilterItem, apiRef: React.MutableRefObject<GridApiCommunity>) => {
...
const filterModelDisableMultiColumnsFilteringWarning = buildWarning(
    [
        'Azrn: The `filterModel` can only contain a single item when the `disableMultipleColumnsFiltering` prop is set to `true`.',
...
const filterModelMissingItemIdWarning = buildWarning(
    'Azrn: The `id` field is required on `filterModel.items` when you use multiple filters.',
    'error'
);

const filterModelMissingItemOperatorWarning = buildWarning(
    'Azrn: The `operator` field is required on `filterModel.items`, one or more of your filtering item has no `operator` provided.',
    'error'
);
...
const getFilterCallbackFromItem = (
    ...
    if (!filterOperators?.length) {
        throw new Error(`Azrn: No filter operators found for column '${column.field}'.`);
    }

    const filterOperator = filterOperators.find((operator) => operator.value === newFilterItem.operator)!;
    if (!filterOperator) {
        throw new Error(
            `Azrn: No filter operator found for column '${column.field}' and operator value '${newFilterItem.operator}'.`
        );
    }
        ...
        outer: for (let v = 0; v < quickFilterValues.length; v += 1) {
            ...
            for (let i = 0; i < appliersPerField.length; i += 1) {
                ...
                if (applier.v7) {
                    ...
                    const cellParams =
                        usedCellParams[field] ?? apiRef.current.getCellParams(getRowId ? getRowId(row) : row.id, field);
