...
export const useGridRowEditing = (
    ...
    const throwIfNotInMode = React.useCallback(
        (id: GridRowId, mode: GridRowModes) => {
            if (apiRef.current.getRowMode(id) !== mode) {
                throw new Error(`Azrn: The row with id=${id} is not in ${mode} mode.`);
            }
        },
        [apiRef]
    );
    ...
    const updateStateToStartRowEditMode = useEventCallback<[GridStartRowEditModeParams], void>((params) => {
    ...
    const updateStateToStopRowEditMode = useEventCallback<[GridStopRowEditModeParams], void>((params) => {
    ...
    const setRowEditingEditCellValue = React.useCallback<GridRowEditingPrivateApi['setRowEditingEditCellValue']>(
        (params) => {
