...
function enrichPageRowsWithPinnedRows(apiRef: React.MutableRefObject<GridApiCommunity>, rows: GridRowEntry[]) {
...
export const useGridKeyboardNavigation = (
    ...
    const handleColumnHeaderKeyDown = React.useCallback<GridEventListener<'columnHeaderKeyDown'>>(
        (params, event) => {
            const headerTitleNode = event.currentTarget.querySelector(`.columnHeaderTitleContainerContent`);
            const isFromInsideContent = !!headerTitleNode && headerTitleNode.contains(event.target as Node | null);
            ...
            switch (event.key) {
                ...
                case 'PageDown': {
                    if (firstRowIndexInPage !== null && lastRowIndexInPage !== null) {
                        goToCell(
                            colIndexBefore,
                            getRowIdFromIndex(Math.min(firstRowIndexInPage + viewportPageSize, lastRowIndexInPage))
                        );
                    }
                    break;
                }
    ...
    const handleHeaderFilterKeyDown = React.useCallback<GridEventListener<'headerFilterKeyDown'>>(
        ...
        [apiRef, currentPageRows.length, goToHeaderFilter, goToHeader, goToCell, getRowIdFromIndex]
    );
    ...
    const handleCellKeyDown = React.useCallback<GridEventListener<'cellKeyDown'>>(
        ...
        [apiRef, currentPageRows, goToCell, getRowIdFromIndex, headerFilteringEnabled, goToHeaderFilter, goToHeader]
    );
