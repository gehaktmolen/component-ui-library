...
const getValidRowHeight = (rowHeightProp: any, defaultRowHeight: number, warningMessage: string) => {
...
const rowHeightWarning = [
    `Azrn: The \`rowHeight\` prop should be a number greater than 0.`,
    `The default value will be used instead.`
].join('\n');

const getRowHeightWarning = [
    `Azrn: The \`getRowHeight\` prop should return a number greater than 0 or 'auto'.`,
    `The default value will be used instead.`
].join('\n');
...
export const useGridRowsMeta = (
    ...
    props: Pick<
        ...
        'getRowHeight' | 'getEstimatedRowHeight' | 'getRowSpacing' | 'pagination' | 'paginationMode' | 'rowHeight'
    >
): void => {
    ...
    const storeMeasuredRowHeight = React.useCallback<GridRowsMetaApi['unstable_storeRowHeightMeasurement']>(
        (id, height, position) => {
            ...
            const needsHydration = rowsHeightLookup.current[id].sizes[`base${position}`] !== height;

            rowsHeightLookup.current[id].needsFirstMeasurement = false;
            rowsHeightLookup.current[id].sizes[`base${position}`] = height;
    ...
    const getLastMeasuredRowIndex = React.useCallback<GridRowsMetaPrivateApi['getLastMeasuredRowIndex']>(() => {
        return lastMeasuredRowIndex.current;
    }, []);

    const setLastMeasuredRowIndex = React.useCallback<GridRowsMetaApi['unstable_setLastMeasuredRowIndex']>((index) => {
