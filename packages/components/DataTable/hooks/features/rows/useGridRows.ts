...
import { GridRowId, GridGroupNode, GridLeafNode, GridRowModelUpdate } from '../../../models/gridRows';
...
export const useGridRows = (
    ...
    props: Pick<
        ...
        'rows' | 'getRowId' | 'rowCount' | 'throttleRowsMs' | 'signature' | 'pagination' | 'paginationMode' | 'loading'
    >
): void => {
    ...
    const updateRows = React.useCallback<GridRowApi['updateRows']>(
        (updates) => {
            if (props.signature === GridSignature.DataGrid && updates.length > 1) {
                throw new Error(
                    [
                        "Azrn: You can't update several rows at once in `apiRef.current.updateRows` on the DataGrid.",
    ...
    const getRowsCount = React.useCallback<GridRowApi['getRowsCount']>(() => gridRowCountSelector(apiRef), [apiRef]);

    const getAllRowIds = React.useCallback<GridRowApi['getAllRowIds']>(() => gridDataRowIdsSelector(apiRef), [apiRef]);

    const getRowIndexRelativeToVisibleRows = React.useCallback<GridRowApi['getRowIndexRelativeToVisibleRows']>(
        (id) => lookup[id],
        [lookup]
    );
    ...
    const setRowChildrenExpansion = React.useCallback<GridRowProApi['setRowChildrenExpansion']>(
        (id, isExpanded) => {
            ...
            if (!currentNode) {
                throw new Error(`Azrn: No row with id #${id} found`);
            }

            if (currentNode.type !== 'group') {
                throw new Error('Azrn: Only group nodes can be expanded or collapsed');
            }
    ...
    const setRowIndex = React.useCallback<GridRowProApi['setRowIndex']>(
        (rowId, targetIndex) => {
            ...
            if (!node) {
                throw new Error(`Azrn: No row with id #${rowId} found`);
            }

            if (node.parent !== GRID_ROOT_GROUP_ID) {
                throw new Error(`Azrn: The row reordering do not support reordering of grouped rows yet`);
            }

            if (node.type !== 'leaf') {
                throw new Error(`Azrn: The row reordering do not support reordering of footer or grouping rows`);
            }
    ...
    const replaceRows = React.useCallback<GridRowApi['unstable_replaceRows']>(
        (firstRowToRender, newRows) => {
            if (props.signature === GridSignature.DataGrid && newRows.length > 1) {
                throw new Error(
                    [
                        "Azrn: You can't replace rows using `apiRef.current.unstable_replaceRows` on the DataGrid.",
                        'You need to upgrade to DataGridPro or DataGridPremium component to unlock this feature.'
                    ].join('\n')
                );
            }
    ...
    const handleStrategyActivityChange = React.useCallback<GridEventListener<'strategyAvailabilityChange'>>(() => {
