 ...
 */
export const hydrateColumnsWidth = (rawState: GridColumnsRawState, viewportInnerWidth: number): GridColumnsState => {
...
export const applyInitialState = (
    ...
    const newOrderedFields =
        cleanOrderedFields.length === 0
            ...
            : [...cleanOrderedFields, ...columnsState.orderedFields.filter((field) => !orderedFieldsLookup[field])];
...
export const createColumnsState = ({
    ...
    if (keepOnlyColumnsToUpsert && !isInsideStateInitializer) {
        columnsToKeep = Object.keys(columnsState.lookup).reduce((acc, key) => ({ ...acc, [key]: false }), {});
    }
...
export function getTotalHeaderHeight(apiRef: React.MutableRefObject<GridApiCommunity>, headerHeight: number) {
