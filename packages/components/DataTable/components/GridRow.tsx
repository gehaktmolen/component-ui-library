...
import { composeClasses, generateUtilityClass, useClassNamesOverride, useForkRef } from '../../../utils';
...
function EmptyCell({ width }: { width: number }) {
    ...
    return <div className={`cell withBorderColor`} style={style} />;
}
...
const GridRow = React.forwardRef<HTMLDivElement, GridRowProps>(function GridRow(props, refProp) {
    ...
    const publishClick = React.useCallback(
        (event: React.MouseEvent<HTMLDivElement>) => {
            const cell = findParentElementFromClassName(event.target as HTMLDivElement, 'cell');
    ...
    const getCell = (
        ...
        cellProps: Pick<GridCellProps, 'width' | 'colSpan' | 'showRightBorder' | 'indexRelativeToAllColumns'>
    ) => {
        const disableDragEvents =
            (disableColumnReorder && column.disableReorder) ||
            (!rowReordering && !!sortModel.length && treeDepth > 1 && Object.keys(editRowsState).length > 0);
    ...
    return (
        <div
            ...
            data-rowindex={index}
            role="row"
            className={twMerge(...rowClassNames, classes.root, className)}
