...
const useUtilityClasses = (ownerState: OwnerState) => {
    const { colDef, isDragging, sortDirection, showRightBorder, filterItemsCounter } = ownerState;
...
function GridColumnHeaderItem(props: GridColumnHeaderItemProps) {
    ...
    const publish = React.useCallback(
        (eventName: keyof GridColumnHeaderEventLookup) => (event: React.SyntheticEvent) => {
            ...
            apiRef.current.publishEvent(eventName, apiRef.current.getColumnHeaderParams(colDef.field), event as any);
        },
        [apiRef, colDef.field]
    );
    ...
    const columnTitleIconButtons = (
        <React.Fragment>
            ...
            {colDef.sortable && !colDef.hideSortIcons && (
                <GridColumnHeaderSortIcon direction={sortDirection} index={sortIndex} sortingOrder={sortingOrder} />
            )}
        </React.Fragment>
    );
