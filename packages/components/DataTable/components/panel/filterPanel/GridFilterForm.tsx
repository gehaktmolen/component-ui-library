...
const useUtilityClasses = () => {
    const slots = {
        root: ['flex p-1'],
        deleteIcon: ['flex-shrink-0 justify-end mr-0.5 mb-0.2'],
        logicOperatorInput: ['min-w-[55px] mr-0.5 justify-end'],
        columnInput: ['w-[150px]'],
        operatorInput: ['width-[120px]'],
        valueInput: ['width-[190px]']
    };
...
const getLogicOperatorLocaleKey = (logicOperator: GridLogicOperator) => {
    switch (logicOperator) {
        ...
        default:
            throw new Error('Azrn: Invalid `logicOperator` property in the `GridFilterPanel`.');
    }
};
...
const GridFilterForm = React.forwardRef<HTMLDivElement, GridFilterFormProps>(function GridFilterForm(props, ref) {
    const {
        item,
        hasMultipleFilters,
        deleteFilter,
    ...
    const baseFormLabelProps = rootProps.slotProps?.baseFormLabel || {};
    ...
    const changeColumn = React.useCallback(
        (event: SelectChangeEvent) => {
            ...
            const eraseItemValue =
                !newOperator.InputComponent || newOperator.InputComponent !== currentOperator?.InputComponent;
    ...
    return (
        <div ref={ref} className={classes.root} data-id={item.id} ownerState={rootProps} {...other}>
            <div
                variant="standard"
            ...
            <div
                ...
                className={classes.logicOperatorInput}
                // className={clsx(
                        ...
                        >
                            {operator.label ||
                                apiRef.current.getLocaleText(
                                    `filterOperator${operator.value}` as 'filterOperatorContains'
                                )}
                        </rootProps.slots.baseSelectOption>
                    ))}
                </rootProps.slots.baseSelect>
