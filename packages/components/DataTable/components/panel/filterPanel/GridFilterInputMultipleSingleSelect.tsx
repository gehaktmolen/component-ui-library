...
export interface GridFilterInputMultipleSingleSelectProps
    extends Omit<
            SelectProps<ValueOptions, true, false, true>,
            | 'options'
            | 'renderInput'
            | 'onChange'
            | 'value'
...
function GridFilterInputMultipleSingleSelect(props: GridFilterInputMultipleSingleSelectProps) {
    ...
    const filteredValues = React.useMemo(() => {
        ...
        if (resolvedValueOptions !== undefined) {
            ...
            return itemValueIndexes.filter((index) => index >= 0).map((index: number) => resolvedValueOptions[index]);
        }
        return item.value;
    ...
    const handleChange = React.useCallback<NonNullable<SelectProps<ValueOptions, true, false, true>['onChange']>>(
        (event, value) => {
    ...
    return (
        <Select<ValueOptions, true, false, true>
            multiple
