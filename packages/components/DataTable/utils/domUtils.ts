...
export function getRowEl(cell?: Element | null): HTMLElement | null {
    ...
    return findParentElementFromClassName(cell as HTMLDivElement, 'row')! as HTMLElement;
}

// TODO remove
export function isGridCellRoot(elem: Element | null): boolean {
    return elem != null && elem.classList.contains('cell');
}

export function isGridHeaderCellRoot(elem: Element | null): boolean {
    return elem != null && elem.classList.contains('columnHeader');
}
...
function getGridRowElementSelector(id: GridRowId): string {
    return `.row[data-id="${escapeOperandAttributeSelector(String(id))}"]`;
}
...
export function getGridCellElement(root: Element, { id, field }: { id: GridRowId; field: string }) {
    ...
    const cellSelector = `.cell[data-field="${escapeOperandAttributeSelector(field)}"]`;
