...
export function useSelect<OptionValue, Multiple extends boolean = false>(
    ...
    const getItemId = React.useCallback((itemValue: OptionValue) => options.get(itemValue)?.id, [options]);
    ...
    if (props.multiple) {
        ...
        selectValue = (selectedOptions.length > 0 ? selectedOptions[0] : null) as SelectValue<OptionValue, Multiple>;
    }
