...
function focusThumb({
    ...
    sliderRef: React.RefObject<HTMLSpanElement | undefined>;
    activeIndex: number;
    ...
    if (
        ...
        sliderRef.current?.querySelector<HTMLInputElement>(`[type="range"][data-index="${activeIndex}"]`)?.focus();
    }
...
export function useSlider(parameters: UseSliderParameters): UseSliderReturnValue {
    ...
    const handleChange =
        ...
        ((event: Event | React.SyntheticEvent, value: number | number[], thumbIndex: number) => {
            ...
            const clonedEvent: Event = new nativeEvent.constructor(nativeEvent.type, nativeEvent);
    ...
    const createHandleHiddenInputChange =
        (otherHandlers: Record<string, React.EventHandler<any>>) => (event: React.ChangeEvent) => {
            ...
            newValue = clamp(<number>newValue, min, max);
            ...
            if (handleChange && !areValuesEqual(newValue, valueDerived as number | number[])) {
    ...
    const handleTouchMove = useEventCallback((nativeEvent: TouchEvent | MouseEvent) => {
        ...
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore buttons doesn't exist on touch event
    ...
    const createHandleMouseDown =
        (otherHandlers: Record<string, React.EventHandler<any>>) => (event: React.MouseEvent<HTMLSpanElement>) => {
    ...
    const getThumbProps = <TOther extends EventHandlers = NonNullable<unknown>>(
        otherHandlers: TOther = {} as TOther
    ...
    const getHiddenInputProps = <TOther extends EventHandlers = NonNullable<unknown>>(
        otherHandlers: TOther = {} as TOther
