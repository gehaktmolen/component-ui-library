...
<Canvas sourceState="hidden" of={SelectDefaultStories.Custom} />
...
The `slots` prop is available on all non-utility Base components. See [Overriding component structure](/docs/docs-intro--docs#overriding-component-structure) for full details.
...
By default, the Select popup is rendered in a [Portal](/docs/utils-portal--docs) and appended to the bottom of the DOM. To instead render the popup where the component is defined, override the `disablePortal` prop of the underlying Popper, as shown below:
...
Hooks do not support [slot props](/docs/docs-intro--docs#the-slotprops-prop), but they do support [customization](#customization) props.
...
The following example shows a select built with a hook. Note how this component does not include any built-in classes. The resulting HTML is much smaller compared to the un-styled component version, as the class names are not applied.
