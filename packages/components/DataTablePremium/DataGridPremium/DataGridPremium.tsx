...
// import { useLicenseVerifier, Watermark } from '../../DataTablePro';
...
import { DataGridProVirtualScroller } from '../../DataTablePro/internals';
    ...
    return (
        <GridContextProvider privateApiRef={privateApiRef} props={props}>
            <GridRoot className={props.className} style={props.style} sx={props.sx} ref={ref} {...props.forwardedProps}>
                <GridHeader />
                <GridBody VirtualScrollerComponent={DataGridProVirtualScroller} ColumnHeadersProps={{ pinnedColumns }}>
...
DataGridPremiumRaw.propTypes = {
    ...
    checkboxSelectionVisibleOnly: chainPropTypes(PropTypes.bool, (props: any) => {
        if (!props.pagination && props.checkboxSelectionVisibleOnly) {
            return new Error(
                'Azrn: The `checkboxSelectionVisibleOnly` prop has no effect when the pagination is not enabled.'
            );
        }
        return null;
    }),
    ...
    filterMode: chainPropTypes(PropTypes.oneOf(['client', 'server']), (props: any) => {
        if (props.treeData && props.filterMode === 'server') {
            return new Error('Azrn: The `filterMode="server"` prop is not available when the `treeData` is enabled.');
        }
        return null;
    }),
    /**
    ...
    hideFooterRowCount: chainPropTypes(PropTypes.bool, (props: any) => {
        if (props.pagination && props.hideFooterRowCount) {
            return new Error('Azrn: The `hideFooterRowCount` prop has no effect when the pagination is enabled.');
        }
        return null;
    }),
    /**
