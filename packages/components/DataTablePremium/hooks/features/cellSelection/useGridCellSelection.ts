...
export const useGridCellSelection = (
    ...
    const setCellSelectionModel = React.useCallback<GridCellSelectionApi['unstable_setCellSelectionModel']>(
        (newModel) => {
            if (!props.unstable_cellSelection) {
                return;
            }
    ...
    const getSelectedCellsAsArray = React.useCallback<GridCellSelectionApi['unstable_getSelectedCellsAsArray']>(() => {
        ...
        return Object.entries(model).reduce<{ id: GridRowId; field: string }[]>(
            (acc, [id, fields]) => [
                ...acc,
                ...Object.entries(fields).reduce<{ id: GridRowId; field: string }[]>((acc2, [field, isSelected]) => {
    ...
    const handleMouseUp = useEventCallback(() => {
        ...
        apiRef.current.rootElementRef?.current?.classList.remove('root--disableUserSelection');
    ...
    const handleCellMouseDown = React.useCallback<GridEventListener<'cellMouseDown'>>(
        (params, event) => {
            ...
            apiRef.current.rootElementRef?.current?.classList.add('root--disableUserSelection');
        ...
        (classes, { id, field }) => {
            ...
            if (rowIndex < visibleRows.range.lastRowIndex) {
                ...
                if (!apiRef.current.unstable_isCellSelected(nextRowId, field)) {
                    newClasses.push('cell--rangeBottom');
                }
            } else {
                newClasses.push('cell--rangeBottom');
            }
            ...
            if (columnIndex > 0) {
                ...
                if (!apiRef.current.unstable_isCellSelected(id, previousColumnField)) {
                    newClasses.push('cell--rangeLeft');
                }
            } else {
                newClasses.push('cell--rangeLeft');
            }
            ...
            if (columnIndex < visibleColumns.length - 1) {
                ...
                if (!apiRef.current.unstable_isCellSelected(id, nextColumnField)) {
                    newClasses.push('cell--rangeRight');
                }
            } else {
                newClasses.push('cell--rangeRight');
            }
