...
import { GridExcelExportApi, GridExportExtension, GridExcelExportOptions } from './gridExcelExportInterface';
import {
    buildExcel,
...
export const useGridExcelExport = (
    ...
    const exportDataAsExcel = React.useCallback<GridExcelExportApi['exportDataAsExcel']>(
        async (options = {}) => {
            ...
            if (exceljsPostProcess && process.env.NODE_ENV !== 'production') {
                console.warn(
                    [
                        `Azrn: The exceljsPostProcess option is not supported when a web worker is used.`,
            ...
            if (exceljsPreProcess && process.env.NODE_ENV !== 'production') {
                console.warn(
                    [
                        `Azrn: The exceljsPreProcess option is not supported when a web worker is used.`,
                        'As alternative, pass the callback to the same option in setupExcelExportWebWorker.'
                    ].join('\n')
                );
            }
    ...
    const addExportMenuButtons = React.useCallback<GridPipeProcessor<'exportMenu'>>(
        (initialValue, options: { excelOptions: GridExcelExportOptions & GridExportDisplayOptions }) => {
