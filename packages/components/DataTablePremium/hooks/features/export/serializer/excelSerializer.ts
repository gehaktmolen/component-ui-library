...
const warnInvalidFormattedValue = buildWarning([
    'Azrn: When the value of a field is an object or a `renderCell` is provided, the Excel export might not display the value correctly.',
    'You can provide a `valueFormatter` with a string representation to be used.'
]);

const getFormattedValueOptions = (colDef: GridSingleSelectColDef, valueOptions: ValueOptions[], api: GridApi) => {
            ...
            case 'singleSelect': {
                ...
                if (typeof castColumn.valueOptions === 'function') {
                    ...
                    dataValidation[castColumn.field] = {
                        ...
                        allowBlank: true,
                        formulae: [
                            `"${formattedValueOptions.map((x) => x.toString().replaceAll(',', 'CHAR(44)')).join(',')}"`
                        ]
                    };
                } else {
...
export function serializeColumns(columns: GridStateColDef[], styles: ColumnsStylesInterface): SerializedColumns {
...
export async function getDataForValueOptionsSheet(
    ...
    return candidateColumns.reduce<Record<string, { values: (string | number)[]; address: string }>>((acc, column) => {
...
export async function buildExcel(options: BuildExcelOptions, api: GridPrivateApiPremium): Promise<Excel.Workbook> {
    const {
        columns,
        rowIds,
        includeHeaders,
