...
import * as PortalDefaultStories from './Portal.stories';
...
The Portal component is used internally by the [Modal](?path=/docs/utils-modal--docs) and [Popper](?path=/docs/utils-popper--docs) components.
...
export default function MyApp() {
    return <Portal>{/* children to be rendered outside the current DOM node */}</Portal>;
}
```
...
The following demo shows how a `<span>` nested within a Portal can be appended to a node outside the Portal's DOM hierarchy—click Mount children to see how it behaves:

<Canvas sourceState="hidden" of={PortalDefaultStories.Primary} />
