...
const useUtilityClasses = (ownerState: SliderOwnerState) => {
    ...
    const slots = {
        root: [
            'inline-block relative touch-none',
            disabled
                ? 'opacity-50 cursor-default pointer-events-none text-gray-300 dark:text-gray-600'
                : 'hover:opacity-100 cursor-pointer text-primary-600 dark:text-primary-400',
            // dragging && '',
            // marked && '',
            orientation === 'vertical' ? 'h-full w-1.5 px-4' : 'h-1.5 w-full py-4'
            // track === 'inverted' && '',
            // track === false && ''
        ],
        rail: [
            'block absolute rounded-sm bg-gray-300 dark:bg-gray-600',
            orientation === 'vertical' ? 'h-full w-1' : 'w-full h-1'
        ],
        track: ['block absolute rounded-sm bg-current', orientation === 'vertical' ? 'w-1' : 'h-1'],
        mark: [
            'absolute w-2 h-2 rounded-full',
            'bg-gray-300 dark:bg-gray-600',
            orientation === 'vertical' ? 'left-[44%]' : 'top-[44%] translate-x-[-50%]'
        ],
        markActive: ['bg-primary-600 dark:bg-primary-400'],
        markLabel: [
            'absolute text-xs',
            'text-gray-500 dark:text-gray-400',
            orientation === 'vertical' ? 'left-8' : 'top-8 translate-x-[-50%]'
        ],
        markLabelActive: [''],
        valueLabel: [
            'block absolute text-sm font-medium',
            orientation === 'vertical' ? 'right-6 translate-y-[-10%]' : 'bottom-5'
        ],
        thumb: [
                    ...
                    return (
                        <React.Fragment key={index}>
                            <Mark
                                ...
                                className={twJoin(markProps.className, markActive && classes.markActive)}
                            />
                            {mark.label != null ? (
                                <MarkLabel
                                    aria-hidden
...
Slider.propTypes = {
    ...
    'aria-label': chainPropTypes(PropTypes.string, (props) => {
        ...
        if (range && props['aria-label'] != null) {
            return new Error(
                'Azrn: You need to use the `getAriaLabel` prop instead of `aria-label` when using a range slider.'
            );
        }
    ...
    'aria-valuetext': chainPropTypes(PropTypes.string, (props) => {
        ...
        if (range && props['aria-valuetext'] != null) {
            return new Error(
                'Azrn: You need to use the `getAriaValueText` prop instead of `aria-valuetext` when using a range slider.'
            );
        }
