...
function FocusTrap(props: FocusTrapProps): JSX.Element {
        ...
        if (!rootRef.current.contains(doc.activeElement)) {
            if (!rootRef.current.hasAttribute('tabIndex')) {
                if (process.env.NODE_ENV !== 'production') {
                    console.error(
                        [
                            'Azrn: The modal content node does not accept focus.',
    ...
    React.useEffect(() => {
        ...
        const contain = () => {
            ...
            if (tabbable.length > 0) {
                ...
                // no tabbable elements in the trap focus or the focus was outside the focus trap
            } else {
                rootElement.focus();
            }
        };
    ...
    return (
        <React.Fragment>
            ...
            <div tabIndex={open ? 0 : -1} onFocus={handleFocusSentinel} ref={sentinelEnd} data-testid="sentinelEnd" />
        </React.Fragment>
    );
}
