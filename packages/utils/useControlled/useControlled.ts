...
export function useControlled(
    ...
    if (process.env.NODE_ENV !== 'production') {
        React.useEffect(() => {
            if (isControlled !== (controlled !== undefined)) {
                console.error(
                    [
                        `Azrn: A component is changing the ${
                            isControlled ? '' : 'un'
        ...
        React.useEffect(() => {
            if (!isControlled && defaultValue !== defaultProp) {
                console.error(
                    [
                        `Azrn: A component is changing the default ${state} state of an uncontrolled ${name} after being initialized. ` +
    ...
    const setValueIfUncontrolled = React.useCallback((newValue: unknown) => {
        if (!isControlled) {
            setValue(newValue);
        }
    }, []);
