import { DataGridPremiumProps, GridRowModel } from '../../components/DataTablePremium';
...
export const addTreeDataOptionsToDemoData = (
    ...
    if (data.rows.length > 1000) {
        throw new Error('Azrn: useDemoData tree data mode only works up to 1000 rows.');
    }

    const rowsByTreeDepth: Record<number, { rows: { [index: number]: RowWithParentIndex }; rowIndexes: number[] }> = {};
    const rowsCount = data.rows.length;

    const groupingCol = data.columns.find((col) => col.field === options.groupingField);

    if (!groupingCol) {
        throw new Error('Azrn: The tree data grouping field does not exist');
    }
