...
import { GridColumnVisibilityModel } from '../../components/DataTable';
import { GridDemoData, getRealGridData } from '../services';
import { getCommodityColumns } from '../columns';
import { getEmployeeColumns } from '../columns';
import asyncWorker from '../services/asyncWorker';
import { GridColDefGenerator } from '../services';
import {
...
export const getColumnsFromOptions = (options: ColumnsOptions): GridColDefGenerator[] => {
    ...
    if (options.visibleFields) {
        columns = columns.map((col) => (options.visibleFields?.includes(col.field) ? col : { ...col, hide: true }));
    }
