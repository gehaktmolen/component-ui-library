...
import { isDeepEqual } from '../../components/DataTable/internals';
import { useDemoData, UseDemoDataOptions, getColumnsFromOptions, getInitialState } from './useDemoData';
...
const getRowComparator = (sortModel: GridSortModel | undefined, columnsWithDefaultColDef: GridColDef[]) => {
    if (!sortModel) {
        const comparator = () => 0;
        return comparator;
    }
...
const getFilteredRows = (
    ...
    const valueGetters = filterModel.items.map(({ field }) =>
        simplifiedValueGetter(field, columnsWithDefaultColDef.find((column) => column.field === field) as any)
    );
    const filterFunctions = filterModel.items.map((filterItem) => {
        ...
        const filterOperator: any = colDef.filterOperators.find(({ value }: GridFilterOperator) => operator === value);
...
export const createFakeServer = (dataSetOptions?: Partial<UseDemoDataOptions>, serverOptions?: ServerOptions) => {
